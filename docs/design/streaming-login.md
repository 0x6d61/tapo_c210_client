...
ローカル録画はライブ再生用とは別のVLCJ MediaPlayerを起動し、録画開始時にJavaFXの保存ダイアログで選択したMP4ファイルへ保存する。初期表示フォルダはOSごとのアプリケーションデータディレクトリ配下の`recordings`で、既定ファイル名を自動生成する。録画用Playerの停止はライブ再生の停止と分離する。
