...
保存済みカメラの行を右クリックすると「編集」を表示する。編集画面は接続フォームと同じIPアドレス、ONVIFポート、RTSPポート、ユーザー名、パスワード、ストリーム画質を持つ。パスワードは既存値を表示せず、空欄なら変更せず、入力した場合だけSQLiteの保存値を置き換える。
...
画質を変更した場合は、選択したストリームへ接続できることを確認してから現在のRTSPセッションを停止する。切り替えに失敗した場合は、元の画質の再生を維持して選択値を元へ戻す。録画中は録画対象のストリームが変わらないよう、画質選択を無効化する。
...
接続後のONVIF能力取得はJavaFXのバックグラウンドTaskで行い、失敗してもRTSP映像を維持する。PTZ操作もTaskで実行し、能力確認前はボタンを無効化する。
動体イベントはONVIF PullPoint購読からJavaFX Application Threadへ配送し、ストリーム画面には最新イベントの種別と時刻を表示する。購読に失敗した場合は動体検知だけを利用不可として、映像とPTZを維持する。
ローカル録画はライブ再生用とは別のVLCJ MediaPlayerを起動し、録画開始時にJavaFXの保存ダイアログで選択したMP4ファイルへ保存する。C210のRTSP音声はALAWでMP4へ格納できないため、H.264映像のみを無変換で保存する。初期表示フォルダはOSごとのアプリケーションデータディレクトリ配下の`recordings`で、既定ファイル名を自動生成する。録画用Playerにはダミーの映像出力を指定し、表示用ウィンドウを作らない。録画用Playerの停止はライブ再生の停止と分離する。
